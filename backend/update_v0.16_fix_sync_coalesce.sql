-- ==========================================
-- ACTUALIZACIÓN V0.16 - FIX SYNC COALESCE & DEBT PAYMENTS
-- ==========================================
...
BEGIN
    -- 1. Extraer datos principales del JSON con COALESCE para evitar restricciones NULL
    v_sale_id := (payload->>'id')::UUID;
    v_client_id := (payload->>'client_id')::UUID;
    v_driver_id := (payload->>'driver_id')::UUID;
    v_transaction_date := COALESCE((payload->>'transaction_date')::TIMESTAMPTZ, NOW());
    v_subtotal_sales := COALESCE((payload->>'subtotal_sales')::NUMERIC, 0);
    v_total_returns := COALESCE((payload->>'total_returns')::NUMERIC, 0);
    v_applied_debt := COALESCE((payload->>'applied_debt')::NUMERIC, 0);
    v_final_total := COALESCE((payload->>'final_total')::NUMERIC, v_subtotal_sales - v_total_returns);
    v_payment_cash := COALESCE((payload->>'payment_cash')::NUMERIC, 0);
    v_payment_transfer := COALESCE((payload->>'payment_transfer')::NUMERIC, 0);
    v_payment_account := COALESCE((payload->>'payment_account')::NUMERIC, 0);
    ...
    IF v_existing_status IS NOT NULL THEN
        -- Si la venta ya estaba completada y no hay cambio de estado, retornar idempotente
    ...
    -- 4. Procesar ítems y actualizar stock si existen ítems
    IF payload->'items' IS NOT NULL AND jsonb_array_length(payload->'items') > 0 THEN
        FOR v_item IN 
            SELECT 
