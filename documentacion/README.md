...
5.- **v0.15 (Permisos RPC, Ordenamiento Consistente de Productos & Corrección de Cobro de Deudas Offline):** Otorga permisos `EXECUTE` al rol `anon` para `process_offline_sale`, corrige la restricción `sales_check` en Postgres al procesar cobros de deuda sin productos nuevos (`final_total = subtotal - returns`), preserva saldos y cajones en `useStore` al recargar la app mientras hay ventas pendientes en la cola de sincronización, y unifica el ordenamiento global de productos por `display_order` (ORDEN 1: PAN, ORDEN 2: BISCOCHOS, etc.) en todas las vistas de repartidores y mostrador.
