...
9.- **Módulo Global de Configuración y Restablecimiento de Memoria (SettingsModal):** Se añadió el botón `Configurar` en todos los roles (Admin, Repartidor, Mostrador) con la acción `Actualizar y Limpiar Memoria` y el botón `Enviar App al Inicio (Instalar)` para añadir el acceso directo del PWA a la pantalla principal del celular en Android (Chrome/Edge) e iOS (Safari). Permite desregistrar Service Workers PWA, vaciar IndexedDB (localForage), CacheStorage y localStorage, descargando la versión más reciente del servidor para solucionar conflictos de datos en teléfonos móviles sin necesidad de acceso físico.
